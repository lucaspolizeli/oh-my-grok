...
export async function resolveSelfImprovePaths({ projectRoot = process.cwd(), topic = '', slug = '', sessionId = '' } = {}) {
  ...
  // Falls back to OMG_SESSION_ID env var when explicit arg is absent.
  ...
  const rawSessionId = sessionId && sessionId.trim()
    ? sessionId.trim()
    : (process.env.OMG_SESSION_ID && process.env.OMG_SESSION_ID.trim() ? process.env.OMG_SESSION_ID.trim() : '');
