...
6. If benchmark not set → read `si-benchmark-builder.md` from this skill directory, spawn a custom Agent(model=grok-4.6) with its content as prompt. The agent surveys the repo, creates or wraps a benchmark, validates 3x, and records baseline.
...
10. **Mode exclusivity**: Call python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state list. If autopilot, ralph, or ultrawork is active, refuse to start.
11. Write initial state: `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (mode='self-improve', active=true, iteration=0, started_at=<now>)`
...
Update `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (mode='self-improve', active=true, status="running")`.
...
`python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (mode='self-improve', active=true, iteration=N)` to reset 30min TTL.

### Step 2 — Check Stop Request

Read state via `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state read (mode='self-improve')`.
...
Spawn 1 general-purpose Agent(model=grok-4.6) with the content of `si-researcher.md` as prompt.
...
Spawn N `oh-my-grok:planner`(model=grok-4.6) agents in parallel (N = `number_of_agents` from settings).
...
For each approved plan, spawn `oh-my-grok:executor`(model=grok-4.6) in parallel.
...
4. Print summary report:
   ...
   ```
5. Run `/oh-my-grok:cancel` for clean state cleanup
