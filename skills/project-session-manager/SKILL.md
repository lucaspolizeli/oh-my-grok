...
Automate isolated development environments using git worktrees and tmux sessions with Grok. Enables parallel work across multiple tasks, projects, and repositories.

Canonical slash command: `/oh-my-grok:project-session-manager` (alias: `/oh-my-grok:psm`).
...
8. **Launch Grok** (unless --no-claude):
   ```bash
