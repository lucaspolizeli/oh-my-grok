...
- **Goal workflow boundary:** when a plan compares Grok `/goal`, Ralph, Team, or artifact-only Ultragoal, identify exactly one primary loop authority and use the deterministic conflict policies `refuse`, `adopt_existing`, and `artifact_only` rather than non-deterministic warning handling. `/goal` facts must cite Grok/Anthropic sources only (Grok `/goal` docs: https://code.claude.com/docs/en/goal; xAI Grok Build changelog), and plans MUST NOT claim the `/goal` evaluator independently runs commands or reads files; require surfaced proof evidence before any completion claim.
...
5. **Consult Analyst** (grok-4.6) for hidden requirements, edge cases, and risks
...
- `--architect codex` — replace the Architect agent pass with `omg ask codex --agent-prompt architect "..."` for implementation-heavy architecture review
- `--critic codex` — replace the Critic agent pass with `omg ask codex --agent-prompt critic "..."` for an external review pass before execution
- If the requested provider is unavailable, briefly note that and continue with the default Architect agent/Critic step for that stage
...
- **On entry**: Call `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (mode="ralplan", active=true, session_id=<current_session_id>)` before step 1
- **On handoff to execution** (approval → ralph/team): Call `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (mode="ralplan", active=false, session_id=<current_session_id>)`. Do NOT use `omg state clear` here — `omg state clear` writes a 30-second cancel signal that disables stop-hook enforcement for ALL modes, leaving the newly launched execution mode unprotected.
- **On true terminal exit** (rejection, non-interactive plan output, error/abort): Call `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state clear (mode="ralplan", session_id=<current_session_id>)` — no execution mode follows, so the cancel signal window is harmless.
...
3. **Architect** reviews for architectural soundness using `spawn_subagent(subagent_type="oh-my-grok:architect", ...)`. Architect review **MUST** include: strongest steelman counterargument (antithesis) against the favored option, at least one meaningful tradeoff tension, and (when possible) a synthesis path. In deliberate mode, Architect should explicitly flag principle violations. **Wait for this step to complete before proceeding to step 4.** Do NOT run steps 3 and 4 in parallel. Architect MUST evaluate the same fixed plan snapshot produced by Planner in step 1 without mutating it; Architect output MUST NOT be passed to Critic.
4. **Critic** evaluates against quality criteria using `spawn_subagent(subagent_type="oh-my-grok:critic", ...)`. Critic **MUST** verify principle-option consistency, fair alternative exploration, risk mitigation clarity, testable acceptance criteria, and concrete verification steps. Critic **MUST** explicitly reject shallow alternatives, driver contradictions, vague risks, or weak verification. In deliberate mode, Critic **MUST** reject missing/weak pre-mortem or missing/weak expanded test plan. Run only after step 3 is complete. Critic MUST evaluate the same fixed plan snapshot independently, as a separate, individually awaited Task call; Critic MUST NOT consume or receive the Architect review.
...
7. On Critic approval (with improvements applied): mark the plan status as `pending approval` unless explicit execution approval has already been captured. _(--interactive only)_ If running with `--interactive`, use `ask_user_question` to present the plan with these options:
   ...
   - **Reject** — discard the plan entirely
     If NOT running with `--interactive`, output the final plan marked `pending approval`, call `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state clear (mode="ralplan", session_id=<current_session_id>)`, and stop. Do NOT auto-execute.
8. _(--interactive only)_ User chooses via the structured `ask_user_question` UI (never ask for approval in plain text). If user selects **Reject**, call `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state clear (mode="ralplan", session_id=<current_session_id>)` and stop.
9. On user approval (--interactive only): Call `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (mode="ralplan", active=false, session_id=<current_session_id>)` **before** invoking the execution skill (ralph/team), so the stop hook does not interfere with the execution mode's own enforcement. Do NOT use `omg state clear` here — it writes a cancel signal that disables enforcement for the newly launched mode.
   - **Approve execution via team**: **MUST** invoke `/oh-my-grok:team` with the approved plan path from `.omg/plans/` as context. Do NOT implement directly. The team skill coordinates parallel agents across the staged pipeline for faster execution on large tasks. This is the recommended default execution path.
   - **Approve execution via ralph**: **MUST** invoke `/oh-my-grok:ralph` with the approved plan path from `.omg/plans/` as context. Do NOT implement directly. Do NOT edit source code files in the planning agent. The ralph skill handles execution via ultrawork parallel agents.
   - **Compact then return for execution approval**: First invoke ``/compact`` to compress the context window (reduces token usage accumulated during planning), then return with the saved pending-approval plan path and require a fresh explicit execution approval before any ralph/team launch. This path is recommended when the context window is 50%+ full after the planning session.

### Review Mode (`--review`)

1. Read plan file from `.omg/plans/`
2. Evaluate via Critic using `spawn_subagent(subagent_type="oh-my-grok:critic", ...)`
...
- Use `spawn_subagent(subagent_type="oh-my-grok:planner", ...)` for planning validation on large-scope plans
- Use `spawn_subagent(subagent_type="oh-my-grok:analyst", ...)` for requirements analysis
- Use `spawn_subagent(subagent_type="oh-my-grok:critic", ...)` for plan review in consensus and review modes
...
- In consensus mode with `--interactive`, on explicit user approval **MUST** invoke `/oh-my-grok:ralph` or `/oh-my-grok:team` for execution (step 9) -- never implement directly in the planning agent
...
- When user selects "Compact then return for execution approval" in step 7 (--interactive only): keep the plan marked `pending approval`, call `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (mode="ralplan", active=false, current_phase="pending_approval", session_id=<current_session_id>)`, then invoke ``/compact`` to compress the accumulated planning context. After compact, require a fresh explicit execution approval before any ralph/team launch; never auto-start implementation from compact continuation
- **CRITICAL — Consensus mode state lifecycle**: Always deactivate ralplan state before stopping or handing off to execution. Use `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (active=false)` for handoff paths (approval → ralph/team) and `omg state clear` for true terminal exits (rejection, error). Never use `omg state clear` before launching an execution mode — its cancel signal disables stop-hook enforcement for 30 seconds.
  </Tool_Usage>
...
- Consensus mode without `--interactive` outputs the final plan marked `pending approval` and stops; with `--interactive`, requires explicit user approval before any implementation begins. **Always** call `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state clear (mode="ralplan", session_id=<current_session_id>)` before stopping.
- If the user says "just do it" or "skip planning" without explicitly naming an execution path, treat it as a request to end planning: output the current plan/spec/proposal as `pending approval` and ask for explicit execution approval via the structured approval UI. Do NOT invoke `/oh-my-grok:ralph`, mutate files, delegate implementation, commit, push, or open a PR from the planning module until that approval exists.
...
- [ ] In consensus mode: ralplan state deactivated on every exit path — `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (active=false)` for handoff to execution, `omg state clear` for terminal exits (rejection, error, non-interactive stop)
      </Final_Checklist>
