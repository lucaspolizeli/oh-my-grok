...
<Purpose>
Deep Interview implements Ouroboros-inspired Socratic questioning with mathematical ambiguity scoring. It replaces vague ideas with crystal-clear specifications by asking targeted questions that expose hidden assumptions, measuring clarity across weighted dimensions, and refusing to proceed until ambiguity drops below the resolved threshold for this run. The output feeds into a gated pipeline: **deep-interview → plan consensus refinement → pending approval → explicitly approved execution**, ensuring maximum clarity before any mutation starts.
</Purpose>
...
If this raw bundled skill is loaded by Grok's native plugin skill loader through `/oh-my-grok:deep-interview` or `/oh-my-grok:deep-interview`, do not treat that path as permission to skip rendered OMG setup. The user-facing preferred invocation is `/deep-interview`; do not recommend or advertise `/oh-my-grok:deep-interview` as the deep-interview entrypoint. Regardless of invocation path, Phase 0 below remains blocking and must resolve `omc.deepInterview.ambiguityThreshold` from settings before any announcement, state write, question, or ambiguity score.

## Phase 0: Resolve Ambiguity Threshold (blocking prerequisite)

Complete this phase before Phase 1, before brownfield exploration, before `omg state write`, before Round 0, and before any ambiguity scoring. Do not continue if the resolved threshold and source are unknown.

1. **Read threshold settings in precedence order**:
   - User settings: `[$GROK_HOME|~/.grok]/settings.json`
...
2. **Resolve threshold and source**:
   ...
   - Set these run variables exactly: `<resolvedThreshold>`, `<resolvedThresholdPercent>`, and `<resolvedThresholdSource>` (for example `./.claude/settings.json`, `[$GROK_HOME|~/.grok]/settings.json`, or `default`).
...
4. **Carry threshold source forward mechanically**:
   ...
   - Include `threshold_source` in the first `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (mode="deep-interview")` state payload and preserve it on later state updates.
...
3.7. **Artifact path discipline**:
   ...
   - Ephemeral interview artifacts (scoring scratchpads, prompt-safe summaries, transient queues, resume metadata) belong in `.omg/state/` or in `omg state write` state, never in the repo root or arbitrary working files.

4. **Initialize state** via `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write (mode="deep-interview")`:
...
Update interview state with the new round, global scores, per-component `topology.components[].clarity_scores`, `topology.components[].weakest_dimension`, ontology snapshot, and `topology.last_targeted_component_id` via `omg state write`.
...
2. **Write to file**: `.omg/specs/deep-interview-{slug}.md`
   ...
   - For ephemeral artifacts during interview rounds (for example scoring intermediate results, prompt-safe summaries, question queues, or resume metadata), use `.omg/state/` or in-memory state via `omg state write`.
...
**Autoresearch override:** if `--autoresearch` is active, skip the standard execution options below. The only valid bridge is the `/oh-my-grok:autoresearch` handoff described above. The `omc autoresearch` CLI is a hard-deprecated shim and must not be used for execution.
...
1. **Refine with plan consensus (Recommended)**
...
2. **Execute with autopilot**
   ...
   - Action: Invoke `/oh-my-grok:autopilot` with the spec file path as context only after the user explicitly selects this execution option. The spec replaces autopilot's Phase 0 — autopilot starts at Phase 1 (Planning).
...
3. **Execute with ralph**
   ...
   - Action: Invoke `/oh-my-grok:ralph` with the spec file path as the task definition.

4. **Execute with team**
   - Description: "N coordinated parallel agents — fastest execution for large specs"
   - Action: Invoke `/oh-my-grok:team` with the spec file path as the shared plan.

5. **Refine further**
   - Description: "Continue interviewing to improve clarity (current: {score}%)"
   - Action: Return to Phase 2 interview loop.

**IMPORTANT:** On explicit execution selection, **MUST** invoke the chosen skill via ``/oh-my-grok:<skill>``. Do NOT implement directly. The deep-interview agent is a requirements agent, not an execution agent. If oversized initial context was summarized, pass the spec and prompt-safe summary forward, not the raw oversized source material. Without explicit execution selection, stop with the spec marked `pending approval`.

### Approval-Gated Refinement Path (Recommended)

```
Stage 1: Deep Interview          Stage 2: plan consensus       Stage 3: Separate approval
┌─────────────────────┐    ┌───────────────────────────┐    ┌──────────────────────┐
...
2. **plan consensus** gates on *feasibility* — is the approach architecturally sound?
...
- Preserve the ask_user_question path for OMC-native interaction; do not introduce OMX-only structured-question transport into this skill
- Use `spawn_subagent(subagent_type="oh-my-grok:explore", model="grok-4.5")` for brownfield codebase exploration (run BEFORE asking user about codebase)
...
- Use `omg state write` / `omg state read` for interview state persistence; the initial and subsequent deep-interview state payloads must include `threshold_source` alongside `threshold`
- Use `Write` tool to save the final spec to `.omg/specs/deep-interview-{slug}.md` exactly; use `.omg/state/` or `omg state write` for ephemeral artifacts
- Use ``/oh-my-grok:<skill>`` to bridge to execution modes only after explicit execution approval — never implement directly
...
- [ ] Spec file written to `.omg/specs/deep-interview-{slug}.md` exactly; ephemeral artifacts stayed under `.omg/state/` or `omg state write`
...
- [ ] Selected execution mode invoked via `/oh-my-grok:<skill>` only after explicit execution approval (never direct implementation)
...
## Approval-Gated Pipeline: deep-interview → plan → pending approval
...
/deep-interview "vague idea"
  ...
  → User explicitly selects "Refine with plan consensus"
...
```
Vague prompt → ralplan gate → deep-interview (if extremely vague) → plan (with clear spec) → pending approval → explicitly approved execution
```
