...
This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with ask_user_question. Write the result to `${GROK_HOME:-~/.grok}/.omc-config.json`.

### Step 1: Detect Existing Configuration

```bash
CONFIG_FILE="${GROK_HOME:-$HOME/.grok}/.omc-config.json"
...
Config saved to: ~/.grok/.omc-config.json
...
**Options:**
1. **OMG (default)** - Display as "OMC"
2. **Grok** - Display as "Grok"
...
Check if `${GROK_HOME:-~/.grok}/omc_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Read or create `${GROK_HOME:-~/.grok}/omc_config.hook.json` and merge the new settings:
...
```bash
LEGACY_CONFIG="${GROK_HOME:-$HOME/.grok}/omc_config.openclaw.json"
