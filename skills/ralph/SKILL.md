...
- User already has an active Grok `/goal` and only wants that native goal loop monitored -- adopt the existing `/goal` explicitly or use artifact-only Ultragoal notes instead of starting Ralph as a competing persistence loop
  </Do_Not_Use_When>
...
3. **Implement the current story**:
   - Delegate to specialist agents at appropriate tiers:
     - Simple lookups: LOW tier (grok-4.5) -- "What does this function return?"
     - Standard work: MEDIUM tier (grok-4.6) -- "Add error handling to this module"
     - Complex analysis: HIGH tier (grok-4.6) -- "Debug this race condition"
   - If during implementation you discover sub-tasks, add them as new stories to the active PRD file
   - Run long operations in background: Builds, installs, test suites use `background: true`
...
7. **Reviewer verification** (tiered, against acceptance criteria):
   - <5 files, <100 lines with full tests: STANDARD tier minimum (grok-4.6)
   - Standard changes: STANDARD tier (grok-4.6)
   - > 20 files or security/architectural changes: THOROUGH tier (grok-4.6)
   - If `--critic=critic`, use `spawn_subagent(subagent_type="oh-my-grok:critic")` for the approval pass
   - If `--critic=codex`, run the Codex CLI on a cmux pane (`omg.py team spawn --provider codex`) or locally if `codex` is on PATH. The Codex critic prompt MUST include:
   ...
   - **On APPROVAL: immediately proceed to Step 7.5 in the same turn. Do NOT pause to report the verdict to the user — reporting happens only at Step 8 (`/oh-my-grok:cancel`) or on rejection (Step 9). Treating an approved verdict as a reporting checkpoint is a polite-stop anti-pattern.**

7.5 **Mandatory Deslop Pass** (runs unconditionally after Step 7 approval, unless `{{PROMPT}}` contains `--no-deslop`):

- **Invoke the `ai-slop-cleaner` skill via the Skill tool: `/oh-my-grok:ai-slop-cleaner`** — it is a Skill, not an agent. If you mistakenly call it via `spawn_subagent(subagent_type="oh-my-grok:ai-slop-cleaner")`, OMG's PreToolUse hook denies the call with the correct Skill-tool identifier; do not substitute a similarly-named agent. Run in standard mode (not `--review`) on the files changed during the current Ralph session only.
...
8. **On approval**: After Step 7.6 passes (with Step 7.5 completed, or skipped via `--no-deslop`), run `/oh-my-grok:cancel` to cleanly exit and clean up all state files
...
- Use `spawn_subagent(subagent_type="oh-my-grok:architect", ...)` for architect verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Use `spawn_subagent(subagent_type="oh-my-grok:critic", ...)` when `--critic=critic`
- Use a Codex CLI worker when `--critic=codex`. Construct the prompt to include: (a) prd.json acceptance criteria, (b) files changed + related files, (c) explicit optimality question: "Is there a meaningfully simpler, faster, or more maintainable approach that achieves the same acceptance criteria?"
...
- Persist ralph with `python3 "${GROK_PLUGIN_ROOT}/scripts/omg.py" state write --mode ralph --active true` and `state read --mode ralph`
- **Skill vs agent invocation**: skills (e.g. `ai-slop-cleaner`) are invoked via the Skill tool: `/oh-my-grok:ai-slop-cleaner`; agents (e.g. `architect`, `critic`, `executor`) via `spawn_subagent(subagent_type="oh-my-grok:<name>")`. OMG's PreToolUse hook denies a Task/Agent call whose `subagent_type` names a bundled skill and returns the correct Skill-tool identifier — do not substitute a similarly-named agent as a "closest match".
  </Tool_Usage>
...
spawn_subagent(subagent_type="oh-my-grok:executor", model="grok-4.5", prompt="Add type export for UserConfig")
spawn_subagent(subagent_type="oh-my-grok:executor", model="grok-4.6", prompt="Implement the caching layer for API responses")
spawn_subagent(subagent_type="oh-my-grok:executor", model="grok-4.6", prompt="Refactor auth module to support OAuth2 flow")
...
- Stop when the user says "stop", "cancel", or "abort" -- run `/oh-my-grok:cancel`
...
- [ ] `/oh-my-grok:cancel` run for clean state cleanup
</Final_Checklist>

## Parallel session caveats

- **Multi-repo workspace anchor:** drop a `.omg-workspace` marker at the parent directory so multiple sessions across sub-repos share one `.omg/`. Resolution order: `OMG_STATE_DIR > .omg-workspace > git > cwd`. See `docs/REFERENCE.md`.
- **Session id source:** OMG_SESSION_ID env var wins in CLI contexts; hook payload data.session_id wins in hook contexts.
- **Plan id (when applicable):** Two ralph runs in the same workspace will conflict on `prd.json`. Use distinct session IDs (the hook payload session_id is already isolated per Grok session). For parallel ultragoal-backed ralph runs, use `--plan-id`.
- **Parallel verdict:** supported (each session writes its own session-scoped state)

<Advanced>
## Background Execution Rules

**Run in background** (`background: true`):
