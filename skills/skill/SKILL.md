...
2. Scan user skills at `${GROK_HOME:-~/.grok}/skills/omc-learned/`
...
USER SKILLS (~/.grok/skills/omc-learned/):
...
5. **Ask for scope:**
   - `user` → `${GROK_HOME:-~/.grok}/skills/omc-learned/<name>/SKILL.md`
...
✓ Created skill at ~/.grok/skills/omc-learned/custom-logger/SKILL.md
...
1. **Search for skill** in both scopes:
   - `${GROK_HOME:-~/.grok}/skills/omc-learned/<name>/SKILL.md`
   - `.omg/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `${GROK_HOME:-~/.grok}/skills/omc-learned/<name>/`)
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.grok/skills/omc-learned/)
...
```
Skill: custom-logger
Scope: user (~/.grok/skills/omc-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.grok/skills/omc-learned/custom-logger/SKILL.md
...
**Behavior:**
1. **Scan both scopes:**
   - User skills: `${GROK_HOME:-~/.grok}/skills/omc-learned/`
   - Project skills: `.omg/skills/`
...
USER_SKILLS_DIR="${GROK_HOME:-$HOME/.grok}/skills/omc-learned"
...
echo "=== USER-LEVEL SKILLS (~/.grok/skills/omc-learned/) ==="
if [ -d "${GROK_HOME:-$HOME/.grok}/skills/omc-learned" ]; then
  USER_COUNT=$(find "${GROK_HOME:-$HOME/.grok}/skills/omc-learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "${GROK_HOME:-$HOME/.grok}/skills/omc-learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
Then ask for scope:
- **User-level** (~/.grok/skills/omc-learned/) - Available across all projects
...
- `/oh-my-grok:skill list` - Show detailed skill inventory
- `/oh-my-grok:skill add` - Start skill creation (invoke skillify)
- `/oh-my-grok:skill scan` - Scan both skill directories
...
- `/oh-my-grok:skillify` - Extract a skill from current conversation (`/oh-my-grok:learner` is a deprecated alias)
- `/oh-my-grok:note` - Save quick notes (less formal than skills)
- `/oh-my-grok:deepinit` - Generate AGENTS.md codebase hierarchy
...
```
> /oh-my-grok:skill list

Checking skill directories...
✓ User skills directory exists: ~/.grok/skills/omc-learned/
...
- Run `/oh-my-grok:skill list` periodically to review your skill library
