...
- Cancel with `/oh-my-grok:cancel` at any time; progress is preserved for resume
</Execution_Policy>
...
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   ...
   - **Otherwise**: Analyst (grok-4.6) extracts requirements, Architect (grok-4.6) creates technical specification
...
2. **Phase 1 - Planning**: Create an implementation plan from the spec
   ...
   - Architect (grok-4.6): Create plan (direct mode, no interview)
   - Critic (grok-4.6): Validate plan
   - Output: `.omg/plans/autopilot-impl.md`

3. **Phase 2 - Execution**: Implement the plan using Ralph + Ultrawork
   - Executor (grok-4.5): Simple tasks
   - Executor (grok-4.6): Standard tasks
   - Executor (grok-4.6): Complex tasks
...
6. **Phase 5 - Cleanup**: Delete all state files on successful completion
   - Remove `.omg/state/autopilot-state.json`, `ralph-state.json`, `ultrawork-state.json` (plus stale `ultraqa-state.json` if a retired copy exists)
   - Run `/oh-my-grok:cancel` for clean exit
</Steps>

<Tool_Usage>
- Use `spawn_subagent(subagent_type="oh-my-grok:architect", ...)` for Phase 4 architecture validation
- Use `spawn_subagent(subagent_type="oh-my-grok:security-reviewer", ...)` for Phase 4 security review
- Use `spawn_subagent(subagent_type="oh-my-grok:code-reviewer", ...)` for Phase 4 quality review
...
Optional settings in `.grok/omg.jsonc` (project) or `~/.config/oh-my-grok/config.jsonc` (user):
...
or the Grok slash compatibility surface:
...
If autopilot was cancelled or failed, run `/oh-my-grok:autopilot` again to resume from where it stopped.
